...
/**
 * <pre>抽象自定义模式认证转换器</pre>
 *
 * @ClassName： AbstractAuthenticationConverter.java
 * @Author： chenpan
 * @Date：2024/12/7 11:38
 * @Modify：
 */
public abstract class AbstractAuthenticationConverter<T extends OAuth2AuthorizationGrantAuthenticationToken> implements AuthenticationConverter {
    /**
     * 是否支持此convert
     *
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        if (StringUtils.hasText(scope) && parameters.get(OAuth2ParameterNames.SCOPE).size() != 1) {
            OAuth2EndpointUtils.throwError(OAuth2ErrorKeys.INVALID_REQUEST, OAuth2ParameterNames.SCOPE,
